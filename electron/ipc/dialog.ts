    ...
    async (_event, directoryPath: string) => {
      try {
        ...
        if (!directoryPath || typeof directoryPath !== "string" || directoryPath.trim() === "") {
          console.error("[Dialog] Invalid directory path provided:", directoryPath);
    ...
    async (
      ...
      try {
        ...
        if (ext === ".docx") {
          ...
          defaultFilters.push({ name: "Excel Spreadsheets", extensions: ["xlsx"] });
        } else if (ext === ".pptx") {
          defaultFilters.push({ name: "PowerPoint Presentations", extensions: ["pptx"] });
        } else if (ext === ".pdf") {
        ...
        const result = await dialog.showSaveDialog(mainWindow || (undefined as any), {
          title: "Save Document",
          defaultPath: options.defaultPath || path.join(app.getPath("documents"), options.filename),
    ...
    async (
      ...
      try {
        ...
        if (!isPathAllowed(options.filePath)) {
          console.error("[Files] Path validation failed - path outside allowed directories:", options.filePath);
          return {
            success: false,
            error: "Path is outside allowed directories. Files can only be saved within your home directory.",
          };
        }
