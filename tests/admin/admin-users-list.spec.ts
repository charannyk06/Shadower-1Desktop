...
test.describe("Admin Users List", () => {
  ...
  test.describe("Search State Preservation", () => {
    test("should maintain search state when navigating to user detail and back", async ({
      ...
      // Click on a user to navigate to detail page (if any results)
      const userRow = page
      ...
      if (!hasUsers) {
        // No users found, just verify search works
      ...
      // Search input should still contain search term
      const searchValue = await searchInput.inputValue();
      expect(searchValue.toLowerCase()).toContain("test");
    });
  ...
  test.describe("Pagination", () => {
    test("should display pagination controls", async ({ page }) => {
      // Check pagination is present (may not exist if not enough users)
      ...
      if (paginationVisible) {
        ...
        expect(hasPrevOrNext).toBeTruthy();
      } else {
