...
export const resolveStoragePrefix = () => {
  ...
  // Remove leading/trailing slashes and dots - use bounded quantifiers to prevent ReDoS
  // Split into separate operations to avoid alternation backtracking issues
  return raw
    .replaceAll(/^\/{1,100}/g, "") // Remove leading slashes (bounded to prevent ReDoS)
    .replaceAll(/\/{1,100}$/g, "") // Remove trailing slashes (bounded to prevent ReDoS)
    .replaceAll(/\.{1,10}/g, "")
    .trim();
};
