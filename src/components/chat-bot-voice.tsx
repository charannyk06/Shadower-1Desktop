...
import { TextPart, ToolUIPart, getToolName, isToolUIPart } from "ai";
  ...
  const toolMentions = useMemo<ChatMention[]>(() => {
    ...
    if (!agentId) {
      if (allowedMcpServers) {
        const mcpMentions = mcpList
          .filter((v) => {
            return (
              v.id in allowedMcpServers &&
              allowedMcpServers[v.id]?.tools?.length
            );
          })
          .flatMap((v) => {
            ...
            {!message.completed ? (
              ...
              message.parts.map((part, index) => {
                if (part.type === "text") {
                  ...
                  // In AI SDK v6, isToolUIPart returns DynamicToolUIPart | ToolUIPart
                ...
                // Skip custom data events (plan updates, sub-agent events, etc.)
                if ((part.type as string)?.startsWith("data-")) {
                  return null;
                }
