...
export function ToolSelectDropdown({
  ...
  const bindingTools = useMemo<string[]>(() => {
    ...
    // Note: We still count tools even when toolChoice is "none" to show the count in UI
    // The toolChoice only affects whether tools are actually passed to the API

    // Count enabled toolkits (not individual tools) + MCP servers
    const toolkitCount = allowedAppDefaultToolkit?.length ?? 0;

    // Count MCP servers (not individual MCP tools)
    const mcpIds = mcpList.map((v) => v.id);
    const enabledMcpServers = Object.values(
    ...
    const totalCount = toolkitCount + mcpServerCount;

    // Return array with correct length (content doesn't matter, only length is used)
    return totalCount > 0 ? Array(totalCount).fill("") : [];
  }, [
    mentions,
  ...
  const triggerButton = useMemo(() => {
    return (
      ...
      >
        <span className={!bindingTools.length ? "text-muted-foreground" : ""}>
          {agentMention
            ? "Agent"
            : (mentions?.length ?? 0) > 0
              ? "Mention"
