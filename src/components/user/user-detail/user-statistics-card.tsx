...
export function UserStatisticsCard({ stats }: UserStatisticsCardProps) {
    ...
    >
      <CardHeader className="pb-4">
        ...
        <p className="text-sm text-muted-foreground flex items-center gap-2">
          AI model usage for {stats.period}
        </p>
      </CardHeader>
        ...
        {!hasActivity ? (
          ...
          <div className="text-center py-12" data-testid="no-activity-state">
            ...
            <p className="text-sm text-muted-foreground mb-4">
              Start chatting with AI to see your usage statistics
            </p>
            <p className="text-xs text-muted-foreground">
              Usage stats will appear here after you start using AI
            </p>
          </div>
        ) : (
          <>
            ...
            {stats.totalTokens > 0 && (
              <div className="rounded-lg p-3">
                <p className="text-sm text-primary/80">
                  {stats.totalTokens.toLocaleString()} tokens across{" "}
                  {stats.modelStats.length} models this{" "}
                  {stats.period.toLowerCase()}
                  {stats.modelStats[0] && (
                    <>
                      {" "}
                      Most active: {stats.modelStats[0].model} with{" "}
                      {stats.modelStats[0].totalTokens.toLocaleString()} tokens
                    </>
                  )}
                </p>
              </div>
