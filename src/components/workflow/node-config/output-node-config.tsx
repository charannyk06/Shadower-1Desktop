...
export const OutputNodeDataConfig = memo(function ({
  ...
  const outputVariables = useMemo(() => {
    ...
    return data.outputData.map((item) => {
      const key = item?.key;
      const source = item?.source;
      const targetNode = nodes.find((node) => node.data?.id === source?.nodeId);
      const schema = targetNode?.data?.outputSchema
  ...
  const updateOutputVariable = useCallback(
    (
      ...
      updateNodeData(data.id, (node) => {
        ...
        const outputData = prev.outputData || [];
        return {
          outputData: outputData.map((v, i) =>
            i === index ? { ...v, ...item } : v,
          ),
        };
      });
    ...
    (key: string = "") => {
      updateNodeData(data.id, (node) => {
        ...
        const newKey = generateUniqueKey(
          key,
          outputData.map((v) => v.key),
        );
        return {
          outputData: [...outputData, { key: newKey, source: undefined }],
        };
      });
    },
    [data.id],
...
export const OutputNodeDataOutputStack = memo(function ({
  ...
  }, [data?.outputData, data?.outputSchema]);
  ...
  return (
    <div className="flex flex-col gap-1 px-4 mt-4">
      {outputVariables.map((item, index) => {
        return (
          <div
            className="border border-border/50 bg-background/50 text-[10px] rounded px-2 py-1 flex items-center gap-1"
            key={index}
          >
