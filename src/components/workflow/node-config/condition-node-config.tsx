...
export function ConditionNodeDataConfig({
  ...
  const removeElseIfBranch = useCallback(
    (index: number) => {
      ...
      const elseIfBranches = data.branches?.elseIf || [];
      if (elseIfBranches[index]) {
        const connectedEdges = edges
          .filter((edge) => edge.sourceHandle == elseIfBranches[index].id)
          .map((edge) => edge.id);
  ...
  // Early return if branches not initialized
  if (!data.branches) {
...
export function ConditionNodeDataOutputStack({
  ...
  // Early return if branches not initialized
  if (!data.branches?.if || !data.branches?.else) {
    return null;
  }
